...
const leafRejected = (reason?: string, user_id?: string, device_id?: string) => ({
...
const terminalLoudDrop = { kind: "drop", reason: "mls_leaf_rejected", loud: true, successorNeeded: false, ack: true };

test("binding_unverified with both ids → needs_identity (recoverable, NOT acked)", () => {
  assert.deepEqual(classifyEnvelopeError(leafRejected("binding_unverified", "01USER", "dev1")), {
    kind: "needs_identity",
    userId: "01USER",
    deviceId: "dev1",
    ack: false,
...
test("binding_unverified MISSING user_id or device_id → terminal loud drop (nothing to reconcile)", () => {
  assert.deepEqual(classifyEnvelopeError(leafRejected("binding_unverified", undefined, "dev1")), terminalLoudDrop);
  assert.deepEqual(classifyEnvelopeError(leafRejected("binding_unverified", "01USER", undefined)), terminalLoudDrop);
});

test("unknown_identity → terminal loud drop (a reconcile cannot pin a brand-new device)", () => {
  assert.deepEqual(classifyEnvelopeError(leafRejected("unknown_identity", "01USER", "dev1")), terminalLoudDrop);
});
...
test("every hostile leaf reason → terminal loud drop", () => {
  for (const reason of [
    ...
    assert.deepEqual(classifyEnvelopeError(leafRejected(reason, "01USER", "dev1")), terminalLoudDrop, reason);
  }
});

test("FUTURE / unrecognised / missing leaf reason → terminal loud drop (default-closed)", () => {
  assert.deepEqual(classifyEnvelopeError(leafRejected("some_future_reason", "01USER", "dev1")), terminalLoudDrop);
  assert.deepEqual(classifyEnvelopeError(leafRejected(undefined, "01USER", "dev1")), terminalLoudDrop);
});

test("leaf rejection is NEVER a quiet drop and NEVER unacked-transient (no wedge, no silent pass)", () => {
  for (const reason of ["binding_unverified", "unknown_identity", "anything_else", undefined]) {
    for (const ids of [["01USER", "dev1"], [undefined, undefined]] as const) {
...
test("mls_welcome_context_mismatch → its own terminal loud drop arm (T-15, never recoverable)", () => {
  assert.deepEqual(classifyEnvelopeError({ type: "mls_welcome_context_mismatch" }), {
    kind: "drop",
...
test("mls_epoch_gap → park (never ack/skip, invariant 10)", () => {
  assert.deepEqual(classifyEnvelopeError({ type: "mls_epoch_gap", expected: 3, got: 5 }), {
    kind: "park",
    expected: 3,
    got: 5,
    ack: false,
...
test("unrecognised error TYPE → transient error, NOT acked (drain bounds the retries)", () => {
  ...
  assert.deepEqual(classifyEnvelopeError(weird), { kind: "error", error: weird, ack: false });
  const untyped = new Error("ipc broke");
  assert.deepEqual(classifyEnvelopeError(untyped), { kind: "error", error: untyped, ack: false });
});
