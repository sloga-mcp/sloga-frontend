...
test("expectedPosition: paused holds, playing advances at rate", () => {
  ...
  assert.equal(
    expectedPosition({ ...s, ratePermille: 1500 }, 1_002_000),
    63_000,
  );
...
test("in band: no actions", () => {
  const r = reconcile(
    snap(session(), provider({ currentTimeMs: 65_100 }), 1_005_000),
    INITIAL_SYNC_STATE,
  );
...
test("small drift: nudge, then release inside the done band", () => {
  ...
  let r = reconcile(
    snap(session(), provider({ currentTimeMs: 65_500 }), 1_005_000),
    INITIAL_SYNC_STATE,
  );
  ...
  r = reconcile(
    snap(
      session(),
      provider({ currentTimeMs: 65_900, ratePermille: 950 }),
      1_005_500,
    ),
    r.state,
  );
  ...
  r = reconcile(
    snap(
      session(),
      provider({ currentTimeMs: 66_050, ratePermille: 960 }),
      1_006_000,
    ),
    r.state,
  );
  ...
  r = reconcile(
    snap(session(), provider({ currentTimeMs: 65_300 }), 1_005_000),
...
test("large drift: hard seek at the provider's threshold, hold after", () => {
  ...
  r = reconcile(
    snap(
      session(),
      provider({ currentTimeMs: 64_200, capabilities: HLS }),
      1_005_000,
    ),
    INITIAL_SYNC_STATE,
  );
...
test("hard seek while nudging resets the rate", () => {
  const r = reconcile(
    snap(
      session(),
      provider({ currentTimeMs: 70_000, ratePermille: 950 }),
      1_005_000,
    ),
    { ...INITIAL_SYNC_STATE, nudging: true },
  );
...
test("host pause: one pause, then the latch holds until the provider agrees", () => {
  ...
  let r = reconcile(
    snap(paused, provider({ currentTimeMs: 65_000 }), 1_005_000),
    INITIAL_SYNC_STATE,
  );
  ...
  r = reconcile(
    snap(
      paused,
      provider({ state: "buffering", currentTimeMs: 65_050 }),
      1_005_400,
    ),
    r.state,
  );
  ...
  r = reconcile(
    snap(
      paused,
      provider({ state: "paused", currentTimeMs: 65_050 }),
      1_006_000,
    ),
    r.state,
  );
  ...
  r = reconcile(
    snap(
      paused,
      provider({ currentTimeMs: 65_000 }),
      1_005_000 + LATCH_TIMEOUT_MS + 1,
    ),
    r.state,
  );
  assert.deepEqual(r.actions, [{ type: "pause" }]);
...
test("host play: viewer paused → play; viewer stalled → NOT told to play", () => {
  let r = reconcile(
    snap(
      session(),
      provider({ state: "paused", currentTimeMs: 65_000 }),
      1_005_000,
    ),
    INITIAL_SYNC_STATE,
  );
  ...
  r = reconcile(
    snap(
      session(),
      provider({ state: "buffering", currentTimeMs: 65_000 }),
      1_005_000,
    ),
    INITIAL_SYNC_STATE,
  );
  ...
  r = reconcile(
    snap(session(), provider({ state: "cued", currentTimeMs: 0 }), 1_005_000),
    INITIAL_SYNC_STATE,
  );
  ...
  r = reconcile(
    snap(
      session(),
      provider({ state: "ended", currentTimeMs: 65_000 }),
      1_005_000,
    ),
    INITIAL_SYNC_STATE,
  );
...
test("seek while session paused re-asserts pause (YouTube seekTo autoplays from cued)", () => {
  ...
  const r = reconcile(
    snap(paused, provider({ state: "cued", currentTimeMs: 0 }), 1_005_000),
    INITIAL_SYNC_STATE,
  );
  assert.deepEqual(r.actions, [
    { type: "seek", ms: 120_000 },
    { type: "pause" },
  ]);
  assert.equal(r.state.pending?.kind, "pause");
});
...
test("non-nudging provider: seek past 1 s, then 3 s hysteresis", () => {
  let r = reconcile(
    snap(
      session(),
      provider({ currentTimeMs: 63_800, capabilities: DUMB }),
      1_005_000,
    ),
    INITIAL_SYNC_STATE,
  );
  ...
  r = reconcile(
    snap(
      session(),
      provider({ currentTimeMs: 65_300, capabilities: DUMB }),
      1_006_500,
    ),
    r.state,
  );
  ...
  r = reconcile(
    snap(
      session(),
      provider({ currentTimeMs: 67_300, capabilities: DUMB }),
      1_008_600,
    ),
    r.state,
  );
  ...
  r = reconcile(
    snap(
      session(),
      provider({ currentTimeMs: 65_600, capabilities: DUMB }),
      1_005_000,
    ),
    INITIAL_SYNC_STATE,
  );
...
test("host rate change applies when not nudging", () => {
  ...
  const r = reconcile(
    snap(fast, provider({ currentTimeMs: 60_000 }), 1_000_000),
    INITIAL_SYNC_STATE,
  );
...
test("unknown position / error: no actions", () => {
  let r = reconcile(
    snap(session(), provider({ currentTimeMs: null }), 1_005_000),
    INITIAL_SYNC_STATE,
  );
  assert.deepEqual(r.actions, []);
  assert.equal(r.driftMs, null);
  r = reconcile(
    snap(session(), provider({ state: "error" }), 1_005_000),
    INITIAL_SYNC_STATE,
  );
  assert.deepEqual(r.actions, []);
});
